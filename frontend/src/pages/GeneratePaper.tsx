...
const subjectsData = {
  ...
  "2": [
    ...
    "Chemical Thermodynamics", "Solutions", "Redox Reactions and Electrochemistry", "Chemical Kinetics",
  ...
  "3": [
    ...
    "Cell Structure and Function", "Plant Physiology", "Human Physiology", "Reproduction",
...
const GeneratePaper = () => {
  ...
  useEffect(() => {
    if (subject) {
      setAvailableTopics(subjectsData[subject as keyof typeof subjectsData] || []);
      setSelectedTopics([]);
    }
  }, [subject]);
  ...
  const handleGenerate = async () => {
    if (!subject || !difficulty || selectedTopics.length === 0 || !title) return;

    setLoading(true);
    try {
      const response = await axios.post('http://localhost:5000/user/generate', {
        title,
        testparam :{
            subject,
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h1 className="text-2xl font-bold text-teal-700 mb-6">Generate Custom Paper</h1>
        ...
        <div className="space-y-6">
                ...
                {availableTopics.map((topic) => (
                  <div key={topic} className="flex items-center">
                    <button
                      ...
                      disabled={!selectedTopics.includes(topic) && selectedTopics.length >= count}
          ...
          <div className="flex justify-end space-x-4">
            ...
            <button
              ...
              disabled={loading || !subject || !difficulty || selectedTopics.length === 0 || !title}
